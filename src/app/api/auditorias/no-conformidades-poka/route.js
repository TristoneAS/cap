...
/** NC agregadas solo de respuestas Poka Yoke (tabla separada). */
  ...
  try {
    ...
    let sql = `
      SELECT nc.id_tipo_nc,
             nc.nombre,
             COUNT(*) AS total
      FROM auditoria_poka_yoke_respuestas ar
      INNER JOIN auditorias aud ON aud.id_auditoria = ar.id_auditoria
      INNER JOIN auditoria_poka_yoke apk ON apk.id_auditoria = aud.id_auditoria AND apk.exenta = 'no'
    ...
    if (conDetalle) {
      let detSql = `
        ...
        INNER JOIN auditoria_poka_yoke apk ON apk.id_auditoria = aud.id_auditoria AND apk.exenta = 'no'
        INNER JOIN tipos_no_conformidad nc ON nc.id_tipo_nc = ar.id_tipo_nc
        LEFT JOIN preguntas_poka_yoke p ON p.id_pregunta_poka = ar.id_pregunta_poka
    ...
    return jsonError("Error al consultar no conformidades Poka Yoke", 500, error.message);
  }
}
